var rules = {
	...
	'input.datepicker': function(elt) {
		new Control.DatePicker(elt, { icon: 'script/calendar.png' });
	},
	'input.timepicker': function(elt) {
		new Control.DatePicker(elt, { icon: 'script/clock.png', datePicker: false, timePicker: true });
	},
	'input.datetimepicker_sd': function(elt) {
		new Control.DatePicker(elt, { icon: 'script/calendar.png', timePicker: true, timePickerAdjacent: true, use24hrs: true , onSelect: function(selected){ 
				var d = new Date(selected); 
					var d1 = d.getFullYear()+"-"+ (d.getMonth() + 1) +"-"+ d.getDate();// + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
					if(jQuery('.datetimepicker_ed').val()!="")
					{
						var edate = jQuery('.datetimepicker_ed').val().split(" ");

						var m1 = new Date(edate[0]);
						var m2 = new Date(d1);
						//alert(m1 +  " === " + d); 
						if(d < m1)
							alert("Start date cannot be less than from End date.");																																
					}
		
		}});
	},
	'input.datetimepicker_ed': function(elt) {
		new Control.DatePicker(elt, { icon: 'script/calendar.png', timePicker: true, timePickerAdjacent: true, use24hrs: true, 
				onSelect: function(selected){
					
					var d = new Date(selected); 
					var d1 = d.getFullYear()+"-"+ (d.getMonth() + 1) +"-"+ d.getDate();// + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
					var sdate = jQuery('.datetimepicker_sd').val().split(" ");
					var m1 = new Date(sdate[0]);
					var m2 = new Date(d1);
					//alert(m1 +  " === " + d); 
					if(m1 > d)
						alert("End date cannot be less than from Start date.");
				}
			});
	},

	'input.datetimepicker_es': function(elt) {
		new Control.DatePicker(elt, { icon: 'script/calendar.png', locale:'es_AR', timePicker: true });
	},
