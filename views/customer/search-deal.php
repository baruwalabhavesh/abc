<?php
/******** 
@USE : search deal page
@PARAMETER : 
@RETURN : 
@USED IN PAGES : mymerchants.php, popup_for_mymerchant.php , my-deals.php, my-orders.php, qr-location.php, register.php, process_mobile.php, login.php, location_detail.php, myreviews.php, process_qrcode.php, qr.php, facebook_login.php, index.php, mypoints.php, zipcodediv.php, header.php, campaign.php, process.php
*********/
//error_reporting(0);
...
//require_once(LIBRARY."/twitteroauth/twitter_secret.php");
...
//unset($_SESSION['not_supported_geolocation']);
...
if(isset($_COOKIE['curr_address']))
{
  if($_COOKIE['curr_address'] == "yes")
  {
	...
	//////////  if user login  then set currant latitude and longitude ///////
       ...
       //echo "Update customer_user set current_location ='".$current_address_string."' , curr_latitude='".$_COOKIE['mycurrent_lati']."' , curr_longitude='".$_COOKIE['mycurrent_long']."' ,curr_timezone= '".$curr_timezone."'  where id=".$_SESSION['customer_id'];   
       //exit();
...
//exit;
		...
		if(! isset($_REQUEST['category_id']) )
		{
			...
			if(! isset($_REQUEST['miles']))
				...
				$dismile=$_REQUEST['miles'];
			
			
			// 07 10 2013 miles problem always 50 miles
			/*
			...
			// 07 10 2013
			*/
			//echo WEB_PATH.'/process.php?btnGetDealsLocations-copy=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id=0'.'&customer_id='.$cust_s;
			/* $arr=file(WEB_PATH.'/process.php?btnGetDealsLocations-copy=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id=0'.'&customer_id='.$cust_s);
			if(trim($arr[0]) == "")
			{
				unset($arr[0]);
			...
			$marker_total_records =$json->marker_total_records;
                        $all_records = $json->all_records; */
//                        echo  "<pre>";
//                     //   print_r($records_array1);
//                        echo "</pre>";
//                        echo "In not category";
//echo WEB_PATH.'/process.php?btnGetSearchDealLocations=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id=0'.'&customer_id='.$cust_s;
$arr_loc=file(WEB_PATH.'/process.php?btnGetSearchDealLocations=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id=0'.'&customer_id='.$cust_s);
							if(trim($arr_loc[0]) == "")
							{
								unset($arr_loc[0]);
		...
		else
		{
			...
			if($_REQUEST['category_id'] == 0) {
				$arr = array();
                         //     echo WEB_PATH.'/process.php?btnGetDealsLocations-copy=yes&category_id=0&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$cust_s;
				/* $arr=file(WEB_PATH.'/process.php?btnGetDealsLocations-copy=yes&category_id=0&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$cust_s);
        ...
        //echo "aaa ".$total_records1." bbb";
		/* $_SESSION['customer_id'] =122;
		echo $_SESSION['customer_id'];
        */
	if(isset($_SESSION['customer_id'])){
								   $not_found_msg = "<div class='warning'><p>!</p><span class='div_msg_err' style='display: block;'>Sorry, there are no offers within <span id='span_miles'></span> miles from your search location in <span id='span_category'></span>. Please try changing your filter setting or search in different category.</span></div>";
                               }
                               else{
								   $not_found_msg = "<div class='warning'><p>!</p><span class='div_msg_err' style='display: block'>Sorry, there are no offers within <span id='span_miles'></span> miles from your search location in <span id='span_category'></span>. Please try changing your filter setting or search in different category.</span></div>";
                               }
...
<meta name="title" content="Browse ScanFlip - Powering smart savings from local merchants">
<meta itemprop="name" property="og:title" content="Browse ScanFlip - Powering smart savings from local merchants">
<meta name="keywords" content="Scanflip,Recommendation,Reviews, local business, coupons, offers, local offers , savings , discounts, deals,campaigns">
<meta property="fb:app_id" content="654125114605525">
<meta property="og:type" content="website">
<meta property="og:site_name" content="www.scanflip.com">
<meta name="description" itemprop="description" property="og:description" content="Get amazing deals at the best places from Scanflip merchants to eat,shop and play. Earn Scanflip points when you redeem and share campaigns with your friends."/>
<meta itemprop="url" property="og:url" content="<?php  echo $_SERVER['SCRIPT_URI']; ?>">
<meta itemprop="image" property="og:image" content="<?php echo ASSETS_IMG; ?>/c/logo.png">
...
if($total_records1_loc>0 || $total_records1_loc!="" )
{  
?> 
    ...
    function try1(val,campid,locid) {
          ...
          //  alert("In"+campid+"=="+locid);
			...
			//  alert($(val).parent().parent('.deal_blk'));
			 ...
			 /* var cal_index = 0;
			 $('.searchdeal_offers #deal_slider_'+locid +' .show_deal_blk .deal_blk').each(function(){
				if(jQuery(this).attr("campid") == campid)
				{
					alert(jQuery(this).attr("campid")+" == "+campid+"==="+cal_index);
					index_of_cur = cal_index;
					curr_element = jQuery(this);
				}
				cal_index =  cal_index+ 1;
				
			 }) */
			 /* alert(jQuery('.searchdeal_offers #deal_slider_'+locid +' .show_deal_blk .deal_blk[campid="'+campid+'"]').parent().nextAll("li.show_deal_blk").length+"==="+jQuery('.searchdeal_offers #deal_slider_'+locid +' .show_deal_blk .deal_blk[campid="'+campid+'"]').parent().next(".show_deal_blk").attr("campid")); */
			 ...
			 for(var k = index_of_cur ;k < tot_element ;k++)
			 {
				 ...
				 if($('.searchdeal_offers #deal_slider_'+locid +'  .deal_blk:eq(' + k + ')').parent().attr("class") == "show_deal_blk" )
				 {
					if(org_text != $('.searchdeal_offers #deal_slider_'+locid +'  .deal_blk:eq(' + k + ')').find('.dealtitle').attr('mypopupid'))
					{
					 ...
					 arr.push(obj);
				//	 alert("pa6al ="+k);
					// alert($('.searchdeal_offers #deal_slider_'+locid +'  .deal_blk:eq(' + k + ')').attr("campid"));
					}
				}				
			 }
			 for(var k = 0 ;k < index_of_cur ;k++)
			 {
			  if($('.searchdeal_offers #deal_slider_'+locid +'  .deal_blk:eq(' + k + ')').parent().attr("class") == "show_deal_blk" )
			   ...
			   /*  alert($(this).find('.dealtitle').attr('mypopupid')); */
				 arr.push(obj);
				 //alert("agal ="+k);
				 //alert($('.searchdeal_offers #deal_slider_'+locid +' .show_deal_blk .deal_blk:eq(' + k + ')').attr("campid"));
				}  
				}
			 }
...
//                                       <a data-count="none" data-lang="en" class="twitter_link" url="http%3A%2F%2Fwww.scanflip.com%2Fregister.php%3Fcampaign_id%3D549%26l_id%3D91%26share%3Dtrue%26customer_id%3DOTk%3D" href="https://twitter.com/intent/tweet?original_referer=http%3A%2F%2Fwww.scanflip.com%2Fpopup_for_mymerchant.php%3FCampTitle%3DKfc-private-kfc-toronto-camp3%26businessname%3DKFC%26number_of_use%3D3%26new_customer%3D0%26address%3DF4-1000%2BGerrard%2BSt%2BE%26city%3DToronto%26state%3DON%26country%3DUSA%26zip%3DM4M%25203G6%26redeem_rewards%3D12%26referral_rewards%3D12%26o_left%3D2%26expiration_date%3D2013-05-23%25205%3A04%3A00%2520AM%26img_src%3Dhttp%3A%2F%2Fwww.scanflip.com%2Fmerchant%2Fimages%2Fdefault_campaign.jpg%26campid%3D549%26locationid%3D91%26deal_desc%3Ddgdf&amp;text=ScanFlip%20%7C%20Campaign&amp;tw_p=tweetbutton&amp;url=http%3A%2F%2Fwww.scanflip.com%2Fregister.php%3Fcampaign_id%3D549%26l_id%3D91%26share%3Dtrue%26customer_id%3DOTk%3D">Twitter</a> 
  ...
  //   
                             ...
                             jQuery(".fancybox-inner .twitter_link").attr("id","facebook_"+campid+"_"+locid);
                              ...
                              //jQuery(".fancybox-inner .google_plus_link").attr("id","<?php echo WEB_PATH?>/register.php?campaign_id="+campid+"&l_id="+locid+"&share=true&customer_id=<?php if(isset($cust_id_with_url)){echo $cust_id_with_url;}?>");  
							...
							var fb_redirect_url = jQuery(".fancybox-inner .btn_facebook").attr("href");
									...
									jQuery(".fancybox-inner .btn_facebook").attr("href",fb_redirect_url+"?redirect_url=<?php echo WEB_PATH ?>/search-deal.php#@795|82@");
								setCookie("redirecting_url_fb","<?php echo WEB_PATH ?>/search-deal.php#@"+campid+"|"+locid+"@",365);
								
								
								
								jQuery(".fancybox-inner .btn_facebook").attr("href",fb_redirect_url+"?redirect_url=<?php echo WEB_PATH ?>/search-deal.php#@795|82@");
								setCookie("get_url1","/search-deal.php#@"+campid+"|"+locid+"@",365);
								
								
									//	alert(jQuery(".fancybox-inner .btn_facebook").attr());
                                          jQuery(".fancybox-inner #hdn_reload_pre_selection").val(campid+"#"+locid);
...
//        {
          /*  if($Row->location_name !="")
            {
                $busines_name=$Row->location_name;
            }
            else 
            { */
       ...
       // }
                  ...
                  /* if($location_time != "")
                   {
                        ...
                        { 
                            		$status_time = "<span class='loca_open_map'>Open</span>";
				        }
						else {
							$status_time = "<span class='loca_closen_map'>Close</span>";

						}
						//$MARKER_HTML  .= $status_time."</div>";
						$MARKER_HTML  .= "<div style='font-weight:400;clear:both'>".$status_time." : ".$location_time;
                   }
                   else
                   {
                       //$location_str .= "<br/>"."Location Hours: Not Available";
                       $status_time  = "<span class='loca_closen_map'>Close</span>";
					   $MARKER_HTML  .= "<div style='font-weight:400;clear:both'>".$status_time." : Not Available";
                   } */
               ...
               $MARKER_HTML .="&nbsp;&nbsp;&nbsp;<a target='_blank' href='".$maphref."'  class='miles_icon' title='Get Direction'></a></div>";
						...
						<?
							if($count != $total_records1_loc) //echo ",";
							$count++;
                           }
                           else{
				...
				if(locations.length==1)
				 ...
				 function initialize() {
					...
					// by deafult
								...
								//	alert(jQuery("#hdn_reload_pre_selection").val()+"here");
					...
					google.maps.event.addListener(marker, 'click', function() {
						...
						{	
							...
							*/
							...
							//alert(jQuery(".location_tool[locid='"+lid+"']").offset().top);
							//alert(jQuery(".location_tool[locid='"+lid+"']").attr('locid'));
							//alert(jQuery(".location_tool[locid='"+lid+"']").attr('miles'));
							//api.scrollToElement(jQuery(".location_tool[locid='"+lid+"']"));
 ...
 <?php 
 } // if bracket for 721 line number  if($total_records1_loc>0 || $total_records1_loc!="" )
 else 
 { // if(isset($_COOKIE['curr_address']))
		?> 
<!--<link href="<?=WEB_PATH?>/templates/template.css" rel="stylesheet" type="text/css">-->
<script type='text/javascript' src="<?php echo ASSETS_JS; ?>/c/mousewheel.js"></script>
 <script type='text/javascript' src="https://maps.google.com/maps/api/js?sensor=true"></script>
<script type='text/javascript' src="https://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclusterer/src/markerclusterer.js"></script>
<?php 
	if($mlatitude != "" && $mlongitude != "" ) 
...
<?php flush(); ?>
<body class="common" onload="checkCookie()">
<?php
...
?>
	...
	document.getElementById("zip").value = "<?=$current_address_string?>";
	document.getElementById('hdn_serach_value').value = "<?=$current_address_string?>";
						...
						?>
							<div id="filterView" class="location_header">
								<div class="newFilterControls">
									...
									<!-- Distance filter -->
									<div id="shareit-box" disp="0" class="shareit-box">
									...
									<input type="hidden" class="merchant_name" value="<?php echo $busines_name; ?>" />
										<div class="loca_image">
											...
											<?php
												...
												{
											?>											
												<img src="<?php echo ASSETS_IMG; ?>/c/Merchant_Location.png" border="0" />
											<?php
												}
											?>											
										</div>
										...
										?>
											...
											<div class="location_marker_iconn">
											<img src="<?php echo ASSETS_IMG; ?>/m/marker_rounded_grey.png"> </div>
  ...
  <div class="slider-viewport">
	...
	<!-- <img id="image789"  src="http://www.scanflip.com/merchant/images/location/location_1391705042.png" "> -->
		  ...
		  ?>
		   ...
		   <div align="center">
				<img class="table_loader defaul_table_loader" src="<?php echo ASSETS_IMG; ?>/c/128.GIF" style="position: absolute; z-index: 1; top: 120px; left: 868px;display: none;">
			</div>
		...
		?>
			...
			<div class="deallistwrapper displayul" mylistsize="<?php echo $total_records1_loc ?>" style="display:none" >
			
			</div>
			
							<div class="deallistwrapper displayul_all" mylistsize="<?php echo $total_records1_loc ?>" >
<!--    <div class="div_msg" ></div>-->
    <!-- start of T_3 -->
        ...
        ?>
			...
			<div id="imagek" >
				  ...
				  if(isset($_COOKIE['curr_address']))
				  {
					...
					<div class="warning">
						...
						<span class="div_msg div_msg_err">
							<?php			
								$dismile=$_COOKIE['miles_cookie'];
								//Here are some of the cities where Scanflip offers are available,
								//$dismile=100; // zip="K8N 5W6"
								//echo WEB_PATH.'/process.php?btnGetNearestLocations=yes&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$_SESSION['customer_id'];
								$arr123=file(WEB_PATH.'/process.php?btnGetNearestLocations=yes&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$cust_s);
								if(trim($arr123[0]) == "")
								{
									unset($arr123[0]);
								...
								if($total_records123>0)
								{
															...
															echo $client_msg['search_deal']['label_No_Records'];
									echo '<div style="margin-top:5px;">';
									$records_array123 = $json123->records;
									foreach($records_array123 as $Row123)
									{
										echo '<li class="nearest_city_span" onclick="call_to_nearest_search(\''.$Row123->city.','.$Row123->state.','.$Row123->country.'\')">'.$Row123->city.','.$Row123->state.','.$Row123->country.'</li>';
									}
									echo '</div>';
								}
								else
								{
									echo $client_msg['search_deal']['label_Sorry_No_Offers_From_Merchant'];
								}
						   ...
						   <span class="div_msg div_msg_err">
								...
								<?php
									...
									//echo WEB_PATH.'/process.php?btnGetNearestLocations=yes&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$_SESSION['customer_id'];
														if(isset($_SESSION['customer_id']))
														{
															$c_id_s = $_SESSION['customer_id'];
														}
														else{
															$c_id_s = "";
														}
													  $c_id_s = "";
									//echo WEB_PATH.'/process.php?btnGetNearestLocations=yes&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$c_id_s;
									$arr123=file(WEB_PATH.'/process.php?btnGetNearestLocations=yes&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&customer_id='.$c_id_s);
									if(trim($arr123[0]) == "")
									{
										unset($arr123[0]);
										$arr123 = array_values($arr123);
									...
									else
									{
										echo $client_msg['search_deal']['label_Sorry_No_Offers_From_Merchant'];
													...
													//echo "Please enter zipcode or city , country";
													echo $client_msg['search_deal']['label_Enter_Zipcode_City_Country'];
												}
								?>
				...
				<?php
				  ...
				  {
              ?>
			   <div class="warning" style="margin-left:10px;width:100% !important;">
					<div class='warning_inner_div' style="padding-top:10px;"><img src="<?php echo ASSETS_IMG?>/c/wornning.png" alt="" /></div>
				   <span class="div_msg div_msg_err">
							   <?php
					  //echo "Please enter zipcode or city , country";
							   echo $client_msg['search_deal']['label_Enter_Zipcode_City_Country'];
			...
			<div class="recently_view">
				...
				?>
						...
						<div class="div_msg_recently">
								<div class="warning">
								<p style="padding-bottom:0px;font-size:20px;margin-top:-7px;">!</p>
...
<div style="display:none"id="confirmation"> 
	...
	<div style="">
	<!--<img src="<?php echo WEB_PATH ?>/images/reload16X16.png" class="flip_map" style="display:none"/>-->
	...
	<input type="hidden" name="hdn_discount" id="hdn_discount" value="" />
...
?>
	...
	<img align="middle" style="height:60px" class="barcode" src="">
      </div>
      <div class="sharediv" >
	...
	catch(e)
	{
		alert(e);
                   ...
                   <div >
                                   ...
                                   ?>
								   <?php //Bahman 09/29/14 changed data-contentdeeplinkid="/scanflip/register" to data-contentdeeplinkid="/register" ?>
                                     <a href="javascript:void(0)" 
										...
										data-contentdeeplinkid="/register" 
                                ...
                                <div class="mt20" style="">
                                  ...
                                  <div class="left ml5" style="float:left;">
                                      <span class="btn btn_gplus glyph glyph_inverse glyph_gplus" id="g-signin-custom">
                                             <a target="_parent" class="login" href="<?php echo $authUrl ?>" class="btn btn_google glyph glyph_inverse glyph_google">
												google+
											</a>
                                      </span>
                                   </div>
...
$RS = json_decode($JSON);
                        ?>
        <div class="updateprofile" id="updateprofile" style="display:none;padding:11px;overflow-y:scroll" >
                    <div id="modal-login" style="height:310px;text-align:left;width:386px;line-height:17px">
                        ...
                        <div id="form_login">
                                <form class="form_vertical" action="process.php" method="post"  id="Not_Set_Login_Frm">
					...
					<tr style="width:100%;float:left;">
						...
						<td>
							<!--<img id="captcha_image_src" src="captcha.gif" style=""/>-->
							 <img src="get_captcha_c_p.php" alt="" id="captcha_image_src" />
						</td>
						<td>
    ...
    <div id="enter_zipcode_div" style="display:none">
        ...
        <div class="zipcodeerror_section"  style="display:none;margin:10px;" align="center">
            <div><?php echo $client_msg['login_register']['label_Enter_Valid_Current_Location'];?></div>
               <br/>
                 <input type="submit" name="searchdeal_cancle_p" id="searchdeal_cancle_p" onclick="<script>jQuery.fancybox.close(); </script>" value="OK" />
        </div>
        </div>
    </div>
...
try 
{
	...
	jQuery("body").on("click","#Not_Set_Login_Frm_Sububscribe #btn_cancel_forgot",function(event) {
					event.stopPropagation(); event.preventDefault();
		jQuery.fancybox.close();
    });
	jQuery("body").on("click","#Not_Set_Login_Frm #btn_cancel_forgot",function(event) {
					event.stopPropagation(); event.preventDefault();
...
if($("#rservedeals_value").val()!=" ")
 ...
 // alert('is_userprofileset=true&custome_id=<?php if(isset($_SESSION['customer_id']))echo $_SESSION['customer_id']; ?>'+$(this).attr("cid")+'&location_id='+$(this).attr("lid")+'&reload=yes');
     $.ajax({
		  type:"POST",
		  url:'process.php',
		  data :'is_userprofileset=true&custome_id=<?php if(isset($_SESSION['customer_id']))echo $_SESSION['customer_id']; ?>'+$(this).attr("cid")+'&location_id='+$(this).attr("lid"),
                  async:false,
		  success:function(msg)
		  {
					...
					// flag = false;
                       // parent.window.location.href=obj.link;
                        ...
                        jQuery(".fancybox-inner .email_popup_div").css("display","none");
                        jQuery(".fancybox-inner .mainloginclass").css("display","none");
						jQuery(".fancybox-inner #Not_Set_Login_Frm").html(jQuery("#notprofilesetid").html());
						//jQuery("#notprofilesetid").remove();
  ...
  // alert('btnreservedeal=true&campaign_id='+$(this).attr("cid")+'&location_id='+$(this).attr("lid")+'&reload=yes');
  //return false;
   $.ajax({
		  type:"POST",
		  url:'process.php',
		  data :'btnreservedeal=true&campaign_id='+$(this).attr("cid")+'&location_id='+$(this).attr("lid")+'&reload=yes&timestamp='+formattedDate(),
          async:false,
		  success:function(msg)
		  {
			//alert(msg);
...
//	});
	...
	$("body").on("click","#captcha_image",function(event) {
	 ...
	 function change_captcha()
	 {
		...
		jQuery(".fancybox-inner #captcha_image_src").attr("src","get_captcha_c_p.php?rnd=" + Math.random());
	 }
	jQuery('body').on("click",".fancybox-inner #ShowVoucherId",function(event) {
      ...
      //      alert(vid);
        ...
        jQuery('body').on("click",".fancybox-inner #ShowshareId",function(event) {
					event.stopPropagation(); event.preventDefault();
	    // $(".sharediv").show();
             jQuery(".fancybox-inner .sharediv").css("display","block");
	     jQuery(".fancybox-inner .showvocherdiv").css("display","none");
	...
	jQuery(".location_tool[locid='"+ locid +"']").find(".sub_loader").css("display","block");
     $.ajax({
      ...
      infowindowcontent[locid1] = jQuery("#temp_infowiondow").html();
                     ...
                     // alert(1);
					  jQuery("#deal_slider_"+locid1+" .deal_blk[level=2]").css("display","block");
					 // end 2-8-13
		...
		?>
		/*var mlat="<?php if(isset($_COOKIE['mycurrent_lati'])){echo $_COOKIE['mycurrent_lati'];} ?>";
		var mlng="<?php if(isset($_COOKIE['mycurrent_long'])){echo $_COOKIE['mycurrent_long'];} ?>"; */
		//alert(mlat+"=="+mlng);
		
		var mlat=getCookie('mycurrent_lati');
		var mlng=getCookie('mycurrent_long');
					//alert("btnGetSearchDealForLocation=yes&category_id=0&location_id=" + locid1 +"&dismile=50&mlatitude="+mlat+"&mlongitude="+mlng+"&customer_id="+customer_id);
						//alert(jQuery("#locationarea_"+locid1+" .deal_blk").length);
						var old_deal_count=jQuery("#deal_slider_"+locid1+" .deal_blk").length;
						var new_deal_count="";	
						$.ajax({
								type:"POST",
								url:'<?php echo WEB_PATH; ?>/process.php',
								data :"btnGetSearchDealForLocation=yes&category_id=0&location_id=" + locid1 +"&dismile=50&mlatitude="+mlat+"&mlongitude="+mlng+"&customer_id="+customer_id,
								async:false,
								success:function(msg)
								{
									...
									//alert(new_deal_count+"==="+old_deal_count);
									if(new_deal_count>old_deal_count)
									{
										...
										if(jQuery(".searchdeal_offers #campaignlis_"+locid1).length == 0)
										{
											...
											//alert("in if");
											//alert(jQuery(".location_tool[locid="+locid1+"]").length+"==length");
											jQuery(".searchdeal_offers").append("<div style='display:none'  class='campaignlist'  camp_locid='"+locid1+"' id='campaignlis_"+locid1+"' camp_catid='"+selected_cat_id+"' camp_miles='"+miles_cookie+"' >"+obj.DetailHtml+"</div>");
											...
											//jQuery(".location_tool[locid='"+locid1+"'] .loca_total_offers span").text(obj.total_records+" offers");
											//alert(jQuery(".location_tool[locid="+locid1+"]").attr("levels"));
										}
										else{
											...
											jQuery(".location_tool[locid="+locid1+"]").attr("t_l_e",obj.expiring);
											//jQuery(".location_tool[locid='"+locid1+"'] .loca_total_offers span").text(obj.total_records+" offers");
											//alert("in else"+jQuery(".searchdeal_offers #campaignlis_"+locid1).length);
											jQuery(".searchdeal_offers #campaignlis_"+locid1).html("");
											jQuery(".searchdeal_offers #campaignlis_"+locid1).html(obj.DetailHtml);
										}
									}
								}			  
							});
				/*	jQuery(".searchdeal_offers .campaignlist").each(function(){
					...
					jQuery("#campaignlis_"+locid1).css("display","block"); */
							/**** fileterinf data ***************/
				...
				//	alert("In selected category");
					...
					*/
									...
									//	alert(li_length+"un subscribe");
										/*if(li_length > 2)
                                        {*/
									//	alert("in if subscribe");
...
jQuery("body").on("click","#geo_cancle",function(){
 //alert("click");
  /*  jQuery(".fancybox-inner .zipcodeerror_section").css("display","none");
    jQuery(".fancybox-inner .zipcode_section").css("display","block");
     jQuery(".fancybox-inner .geolocation_error").css("display","none"); */
...
*/
jQuery(".ratinn_box").live('mouseenter',function() {
	var p = jQuery(this).position();
	//alert(p.top+"====="+p.left);
...
jQuery(".fancybox-inner .btn_share_facebook").live("click",function(event) {
					event.stopPropagation(); event.preventDefault();
   //alert(this.id);
    ...
    var cust_id="<?php if(isset($_SESSION['customer_id'])){echo $_SESSION['customer_id'];} ?>";
		  ...
		  success:function(msg)
		  {
                      ...
                      var obj = jQuery.parseJSON(msg);
                       ...
                       {
						//alert('campaign_title='+encodeURIComponent(campaign_title)+'&summary='+summary+'&th_link='+th_link+'&imgsrc='+imgsrc+'&customer_id='+cust_id+'&campaign_id='+campaign_id+'&l_id='+l_id+'&campaign_tag='+campaign_tag+'&redirect_url='+redirect_url);		
                           $.ajax({
                                type:"POST",
...
if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
 ...
 //document.write('<script type="text/javascript" src="<?=WEB_PATH?>/js/search-deal-script-mobile.js"><\/script>');
}
else{
     document.write('<script type="text/javascript" src="<?php echo ASSETS_JS ?>/c/newjscrollpane.js"><\/script>');
	// document.write('<script type="text/javascript" src="<?=WEB_PATH?>/js/search-deal-script1.js"><\/script>');
...
jQuery(".call_location_details").click(function(event) {
	...
	*/
	...
	//alert('<?php echo WEB_PATH; ?>/process.php?get_location_details_all_from_ajax=yes&location_id='+location_id+'&business='+businessname+'&address='+address);
	jQuery.ajax({
		  type:"POST",
		  url:'<?php echo WEB_PATH; ?>/process.php',
...
var total_no_of_deals=0;
var no_of_private_deal = 0;
miles_cookie = getCookie("miles_cookie");
selected_cat_id = getCookie("cat_remember");
var offers_content = jQuery(this).text();
/* check whether user login or not and restore previous section */
//if(jQuery("#deal_slider_"+locid).length == 0)
//{
	
	no_of_private_deal = get_private_deals(selected_cat_id,miles_cookie,locid);
	//alert(no_of_private_deal);
	//offers_content = jQuery("location_tool[locid='"+locid+"'] .loca_total_offers span").text();
...
jQuery("#deal_slider_"+locid+" .deal_blk").each(function(){
	if(jQuery(this).parent().css("display") == "none")
	{
	}
	else{
		if(jQuery(this).attr("level") == 2)
		{ 
			no_of_private_deal = no_of_private_deal + 1;
		}
			total_no_of_deals = total_no_of_deals + 1;
	}

});
}*/
//alert(total_no_of_deals+"==="+no_of_private_deal);
if(no_of_private_deal != 0)
{
//alert("in if");
			  ...
			  success:function(msg)
			  {
						  ...
						  var obj = jQuery.parseJSON(msg);
						   if(obj.status == "false")
						   {
								...
								{
									parent.window.location.href= window.location.href+obj.hash_link;
									location.reload();
								}
						   }
						   else
...
var is_discount = jQuery("#hdn_discount").val();
		...
		{
		//open_popuploader('Notificationloader');
		//alert(jQuery(".location_tool[locid='"+locid+"'] .offer_loader").length);
		...
		jQuery.ajax({
			type: "POST",
			url: "<?=WEB_PATH?>/process.php",
			data: "btnGetSearchDealForLocation=yes&category_id=0&location_id=" + locid +"&dismile=50&mlatitude="+mlat+"&mlongitude="+mlng+"&customer_id="+customer_id,
			async:false,
			success: function(msg) 
			{
...
if(jQuery("#rservedeals_value").val()!=" ")
	...
	//alert('<?php echo WEB_PATH; ?>/process.php?btnunreservedeal=true&campaign_id='+$(this).attr("u_campid")+'&l_id='+$(this).attr("u_locid")+'&customer_id=<?php if(isset($_SESSION['customer_id'])){echo $_SESSION['customer_id'];} ?>');
	//alert('btnunreservedeal=true&campaign_id='+$(this).attr("u_campid")+'&l_id='+$(this).attr("u_locid")+'&customer_id=<?php if(isset($customer_id_p)){echo $customer_id_p;} ?>');
 jQuery.ajax({
		  type:"POST",
		  url:'<?php echo WEB_PATH; ?>/process.php',
		  data :'btnunreservedeal=true&campaign_id='+$(this).attr("u_campid")+'&l_id='+$(this).attr("u_locid")+'&customer_id=<?php if(isset($customer_id_p)){echo $customer_id_p; }?>',
          async:false,
		  success:function(msg)
		  {
		  //alert(msg);
		...
		var mlng=getCookie('mycurrent_long');
				...
				//	alert("In selected category");
					...
					else
					{
						...
						{
							jQuery("#campaignlis_"+locid1+" .deal_blk").each(function(){
					...
					*/
					// jQuery("#deal_slider_"+locid1+" .deal_blk[level=2]").parent().removeClass("show_deal_blk");
					 //  jQuery("#deal_slider_"+locid1+" .deal_blk[level=2]").parent().addClass("hide_deal_blk");
                                       // alert(jQuery("#deal_slider_"+locid1+" .deal_blk[level=2]").parent().attr("class"));
                                        jQuery('#campaignlis_'+locid).find("#prev1").css("display","none");
                                        jQuery('#campaignlis_'+locid).find("#next1").css("display","none");
                                        var li_length=jQuery("ul[id='deal_slider_"+locid+"'] li[class='show_deal_blk']").length;
										//jQuery('#deal_slider_'+locid).css("width","530px");
    ...
    <div id="LocationFancyFrontDivProcessing" class="Processing" style="display:none;">
        ...
        <div id="LocationFancyMaindivLoading" align="center" valign="middle" class="imgDivLoading"
			...
			<div class="modal-close-button" style="visibility: visible;">
                            <a tabindex="0" onclick="close_popup('LocationFancy');" id="fancybox-close" style="display:inline;">
                           </a>
                        </div>
...
jQuery.fancybox({
	...
	beforeShow:function(){
		jQuery(".fancybox-inner").css("height","150");
	}
});
*/
var iOS = ( navigator.userAgent.match(/(iPad|iPhone|iPod)/g) ? true : false );
if(iOS)    
{
 ...
 jQuery('<link rel="stylesheet" type="text/css" href="<?=WEB_PATH?>/templates/iosdevicecss.css" />').appendTo("head");
