...
@PARAMETER : 
@RETURN : 
@USED IN PAGES : zipcodediv.php, header.php, process.php
*********/
...
//{
//     $geocode=file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng='.$_COOKIE["mycurrent_lati"] .','.$_COOKIE["mycurrent_long"].'&sensor=false');
...
// to set lati & long in cookie
//echo "123";
//echo WEB_PATH.'/process.php?get_customer_subscribes_merchant=yes&lati='.$mlatitude.'&long='.$mlongitude.'&customer_id='.$_SESSION['customer_id'];
//exit;
			$arr = array();
				if(isset($_SESSION['customer_id']))
				{
	...
	if(isset($_SESSION['customer_id'])){
		...
		$dismile = 50;
		//echo WEB_PATH.'/process.php?new_campaign_counter_for_24_hour=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id='.$category.'&customer_id='.$custid;
		$arr_new_subs_merchant_deal=file(WEB_PATH.'/process.php?new_campaign_counter_for_24_hour=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id='.$category.'&customer_id='.$cust_s);
		if(trim($arr_new_subs_merchant_deal[0]) == "")
		{
			unset($arr_new_subs_merchant_deal[0]);
			$arr_new_subs_merchant_deal = array_values($arr_new_subs_merchant_deal);
		}
		$all_json_str = $arr_new_subs_merchant_deal[0];                   
		$json_deal_24_hour = json_decode($arr_new_subs_merchant_deal[0]);
		$total_records_subs_merchant_24_hour = $json_deal_24_hour->total_records;
		$records_subs_merchant_24_hour = $json_deal_24_hour->campaign_list;
		$deal_24_array=explode(";",$records_subs_merchant_24_hour);
		
	$dismile = 50;
$cust_s = $_SESSION['customer_id'];
	//echo WEB_PATH.'/process.php?btnGetSubscribedMerchantLocations=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id=0'.'&customer_id='.$cust_s;
	$arr_loc=file(WEB_PATH.'/process.php?btnGetSubscribedMerchantLocations=yes&dismile='.$dismile.'&mlatitude='.$mlatitude.'&mlongitude='.$mlongitude.'&category_id=0'.'&customer_id='.$cust_s);
	if(trim($arr_loc[0]) == "")
	{
		unset($arr_loc[0]);
	...
	$total_records1 = $json_loc->total_records;
...
<head >
<title>ScanFlip | My Subscribed Merchants</title>
...
//                                       <a data-count="none" data-lang="en" class="twitter_link" url="http%3A%2F%2Fwww.scanflip.com%2Fregister.php%3Fcampaign_id%3D549%26l_id%3D91%26share%3Dtrue%26customer_id%3DOTk%3D" href="https://twitter.com/intent/tweet?original_referer=http%3A%2F%2Fwww.scanflip.com%2Fpopup_for_mymerchant.php%3FCampTitle%3DKfc-private-kfc-toronto-camp3%26businessname%3DKFC%26number_of_use%3D3%26new_customer%3D0%26address%3DF4-1000%2BGerrard%2BSt%2BE%26city%3DToronto%26state%3DON%26country%3DUSA%26zip%3DM4M%25203G6%26redeem_rewards%3D12%26referral_rewards%3D12%26o_left%3D2%26expiration_date%3D2013-05-23%25205%3A04%3A00%2520AM%26img_src%3Dhttp%3A%2F%2Fwww.scanflip.com%2Fmerchant%2Fimages%2Fdefault_campaign.jpg%26campid%3D549%26locationid%3D91%26deal_desc%3Ddgdf&amp;text=ScanFlip%20%7C%20Campaign&amp;tw_p=tweetbutton&amp;url=http%3A%2F%2Fwww.scanflip.com%2Fregister.php%3Fcampaign_id%3D549%26l_id%3D91%26share%3Dtrue%26customer_id%3DOTk%3D">Twitter</a> 
  ...
  //   
							...
							// for google plus
                                           ...
                                           setCookie("redirecting_url_fb","<?php echo WEB_PATH ?>/mymerchants.php#@"+campid+"|"+locid+"@",365);
  ...
  <?php
		...
		<div class="warning" >
		   ...
		   <span class="div_msg div_msg_err" >
				<!--Sorry currently there are no offers from your subscribe merchants. Please browse Scanflip to find offers near you.-->				
                               <?php echo $client_msg['mymerchant']['label_filter_area'];?> 
			</span>
	  </div> 
	  <?php
	  }
...
jQuery(document).ready(function(){
			...
			var margin = $("#imager").width()/2;
								...
								$("#imager").stop().animate({width:'0px',height:''+height+'px',marginLeft:''+margin+'px',opacity:'0'},{duration:300,complete:function(){
				$("#imager").css("zIndex","2");
				$("#imager").css("display","none");
				//alert($(this).css("zIndex"));
			}});
		$("#reflectionr").stop().animate({width:'0px',height:''+height+'px',marginLeft:''+margin+'px'},{duration:300});
		window.setTimeout(function() {
		$("#imagek").stop().animate({width:''+width+'px',height:''+height+'px',marginLeft:'0px',opacity:'1'},{duration:300,complete:function(){
				$("#imagek").css("zIndex","3");
				
				//alert($(this).css("zIndex"));
			}});
		$("#reflectionk").stop().animate({width:''+width+'px',height:''+height+'px',marginLeft:'0px'},{duration:300});
		},300); 
	 ...
	 //alert("search deal");
		...
		//	var longitude = document.getElementById("my_longitude").value;
			//document.getElementById("slider_iframe").src = WEB_PATH+"/search-deal-map.php?latitude="+latitude+"&longitude="+longitude+"&category_id="+catid;
                        window.location.href=  WEB_PATH+"/mymerchants.php";
			return false;
		}
...
$RS = json_decode($JSON);
        ...
        <div class="updateprofile" id="updateprofile" style="display:none;padding:11px;" >
                    <div id="modal-login" style="height:310px;text-align:left;width:386px;line-height:17px">
                        ...
                        <div id="form_login">
                                <form class="form_vertical" action="process.php" method="post"  id="Not_Set_Login_Frm_reserve">
...
<div class="unit100" style="height:30px;">
                        ...
                        <div id="form_login">
                                <form class="form_vertical" action="process.php" method="post"  id="Not_Set_Login_Frm_subscribe">
				      <label for="email-modal"><?php echo "* ".$client_msg['login_register']['label_Gender'];?></label>
                                  <select name="gender" id="gender" class="genderclass"   style="padding:0.231em;margin:0 0 0.154em;" class="js-focus"  >
				        <option></option>
                                        <option value="1" <?php if($RS[0]->gender == 1){ echo "selected";} ?> >Male</option>
...
$(".fancybox-inner .btn_mymerchantreserve").live("click",function(event) {
					event.stopPropagation(); event.preventDefault();
...
if($("#rservedeals_value").val()!=" ")
    ...
    //if(jQuery("#is_profileset").val() != 1){
     $.ajax({
		  type:"POST",
		  url:'process.php',
		  data :'is_userprofileset=true&custome_id=<?php echo $_SESSION['customer_id']; ?>'+$(this).attr("cid")+'&location_id='+$(this).attr("lid")+'&reload=yes',
                  async:false,
		  success:function(msg)
		  {
...
//          jQuery(".fancybox-inner .cust_attr_tooltip").css("display","none");
//	});
        $("body").on("click",".fancybox-inner #Notificationlogin",function(){
...
jQuery(".fancybox-inner .btn_share_facebook").live("click",function(){
   ...
   var data_array=jQuery(this).attr('data').split('###');
        ...
        var redirect_url="<?php echo WEB_PATH."/mymerchants.php";?>";
	//alert(redirect_url);
...
var is_discount = jQuery("#hdn_discount").val();
		...
		jQuery.ajax({
			type: "POST",
			url: "<?=WEB_PATH?>/process.php",
			data: "btnGetSubscribedMerchantForLocation=yes&category_id=0&location_id=" + locid +"&dismile=50&mlatitude="+mlat+"&mlongitude="+mlng+"&customer_id="+customer_id,
			async:false,
			success: function(msg) 
			{
				var obj=jQuery.parseJSON(msg);
...
/*		
		if(iOS)
		{
		//alert(jQuery(".campaignlist .dealtitle").length);
			jQuery(".campaignlist .dealtitle").each(function(){
				jQuery(this).attr("onmouseover",jQuery(this).attr("onClick"));
				//alert(jQuery(this).attr("onmouseover"));
				jQuery(this).attr("onClick","");
			});
			jQuery(".campaignlist .view-reserve").each(function(){
				jQuery(this).attr("linkurl",jQuery(this).attr("href"));
...
jQuery(".loca_categories span").live("click",function(event) {
	...
	if(jQuery(".searchdeal_offers #campaignlis_"+selected_location).length == 0)
	{
		...
		jQuery.ajax({
			type: "POST",
			url: "<?=WEB_PATH?>/process.php",
			data: "btnGetSubscribedMerchantForLocation=yes&category_id=0&location_id=" + selected_location +"&dismile=50&mlatitude="+mlat+"&mlongitude="+mlng+"&customer_id="+customer_id,
			async:false,
			success: function(msg) 
			{
	...
	jQuery(".location_tool[locid='"+ locid1 +"']").find(".sub_loader").css("display","block");
     $.ajax({
      ...
      infowindowcontent[locid1] = jQuery("#temp_infowiondow").html();
					...
					jQuery(".location_tool[locid='"+locid1+"']").detach();
