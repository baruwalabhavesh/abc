...
$RS = $objDB->Show("giftcards", $array_where);
 ...
 <div id="container">
	...
	<form action="process.php" method="post">
              <input type="hidden" name="hdn_image_path" id="hdn_image_path" value="<?php echo $RS->fields['image']; ?>" />
                <input type="hidden" name="hdn_id" id="hdn_id" value="<?php echo $RS->fields['id']; ?>" />
		<table border="0" cellspacing="2" cellpadding="2" >
		   <tr>
					<td width="20%" align="right">Giftcard Title : </td>
					<td width="80%" align="left">
						<input type="text" name="title" id="title" value="<?php  echo $RS->fields['title']; ?>" />
					</td>
				  </tr>
				<tr>
...
<span class="notification_tooltip" title="Keywords will help customer search for your gift card">&nbsp;&nbsp;&nbsp;</span>
				 ...
				 <tr>
					...
					<td width="80%" align="left">
						<input type="text" name="giftcard_value" id="giftcard_value" value="<?php  echo $RS->fields['card_value']; ?>" />
					</td>
				  </tr>
				  <tr>
					<td width="20%" align="right">Giftcard Redemption Value : </td>
					<td width="80%" align="left">
						<input type="text" name="giftcard_rdem_value" id="giftcard_rdem_value" value="<?php  echo $RS->fields['redeem_point_value']; ?>" />
					</td>
				  </tr>
				  <tr>
					...
					<td width="80%" align="left">
						<select onchange="filter_locations()" id="giftcard_Shipto" name="giftcard_Shipto">
							<option value="USA" <?php if($RS->fields['ship_to'] == 'USA') { echo "selected"; } ?> >USA</option>
							<option value="Canada" <?php if($RS->fields['ship_to'] == 'Canada') { echo "selected"; } ?>>CANADA</option>
						</select>
					</td>
				  </tr>
				  <tr>
					...
					?>
						<option value="<?=$Row['id']?>" <?php if($RS->fields['category_id'] == $Row['id'] ) { echo "selected";} else {} ?> ><?=$Row['cat_name']?></option>
					<?
					}
					?>
					</select>
				  ...
				  <td align="left">
											...
											$RS_mer =  $objDB->Conn->Execute($sql);
                                            ?>
                                            <select id="merchant_id" name="merchant_id">
                                                <?php 
                                                while($Row = $RS_mer->FetchRow()){
                                                ?>
                                                <option value="<?php echo $Row['id']?>" <?php if($RS->fields['merchant_id'] == $Row['id'] ) { echo "selected";} ?>><?php echo $Row['merchant_name']; ?> 
				  ...
				  <tr>
					...
					<div class="textarea_container" style="display: none;">	
						<textarea id="description" name="description" rows="15" cols="80" style="width: 80%"><?=htmlentities($RS->fields['description'])?></textarea>
				  ...
				  <tr>
					<td>&nbsp;</td>
					<td align="left">
						<input type="submit" name="btnEditGiftCard" id="btnEditGiftCard" value="Save" onClick="mycall();">
						<!--// 369-->
