...
if(isset($_REQUEST['active']))
{
    ...
    $objDB->Update($array_values, "giftcard_categories", $where_clause);
}

$RS = $objDB->Show("giftcard_categories",''," Order By `order` ASC ");
...
<body>
	...
	<h2>Manage Gift Card Category </h2>
	<!--edit delete icons table-->
	<div><table align="right"  cellspacing="10px" >
	<tr><td><a href="add-giftcardcategory.php"><img src="<?php echo ASSETS_IMG; ?>/a/icon-add.png"></a></td>
	...
	<div style="color:#FF0000; "><?=$_SESSION['msg'];?></div>
		...
		<table width="100%"   class="tableAdmin">
		  ...
		  <tr>
			...
			<a href="edit-giftcardcategory.php?id=<?=$Row['id']?>">Edit</a>&nbsp;|&nbsp;
                        
                            <?php
                                if($Row['active']==1)
                                {
                            ?>
                                    <a href="giftcard_categories.php?active=0&id=<?=$Row['id']?>">Deactivate</a>        
                            <?php                                  
                                }
                                else
                                {
                                ?>
                                    <a href="giftcard_categories.php?active=1&id=<?=$Row['id']?>">Activate</a>        
                            <?php
                                }
                            ?>
                        </a>
