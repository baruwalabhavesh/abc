...
if($_SESSION['merchant_id'] != "")
{
	...
	{	
		if(isset($_REQUEST['activationcode']) && isset($_REQUEST['qrcodeid'])) 
		{
			...
			$records_array = $objDB->Conn->Execute("SELECT * FROM locations l WHERE l.id IN (SELECT cl.location_id FROM campaign_location cl WHERE cl.campaign_id = ?) and l.active=1",array($_REQUEST['id']));
		}
		else 
		{	
	...
	<div id="content">
		<div class="title_header"><?php echo $merchant_msg['marketingmaterial']['marketing_material'];?></div>
                <div id="backdashboard">
					...
					{
					?>
						<a id="dashboard" href="<?=WEB_PATH?>/merchant/manage-marketing-material.php?tab=location"><img src="<?=ASSETS_IMG ?>/m/back_but.png" /></a>
					<?php
					}
					?>
                </div>
			   ...
			   <tr>
				...
				<div class="datatable_container" style="display: none;">
				  ...
				  <tr>
					...
					<tbody>
					   <?
					   //echo $records_array->RecordCount();
						...
						*/ 
                        ...
                        ?>
						  <tr class="tableDeal" style="<?=$css?>">
							 <td>
					<!--                     <input type="checkbox" name="chklocation[]" value="<?php echo $Row['id']; ?>" />-->
					...
					<!--                <td> -->
										...
										?>
										<?=$Row['address'].", ".$RS_city->fields['name'].", ".$RS_state->fields['short_form'].", ".$Row['zip']?></a></td>
        ...
        {                                   
			...
			if(acticationcode=="")
			{
				if(jQuery("#hdn_campaignid").val() ==  0 || jQuery("#hdn_campaignid").val() == "" )
				{
					...
					var head_msg="<div class='head_msg'>Message</div>"
					var content_msg="<div class='content_msg' style='text-align:center'><?php echo $merchant_msg['marketingmaterial']['please_select_location'];?></div>";
					var footer_msg="<div><input type='button'  value='<?php echo $merchant_msg['index']['fancybox_ok'];?>' id='popupcancel' name='popupcancel' class='msg_popup_cancel'></div>";
					jQuery( "#dialog-message" ).html(head_msg + content_msg +footer_msg);
					jQuery.fancybox({
						content:jQuery('#dialog-message').html(),
			...
			//      {
			//          //alert("Enter Text To Print On Your Marketing Material");
			//	  var head_msg="<div style='line-height:2;height:40px;width:100%;border-radius:4px 4px 4px 4px;background:#D5D5D5;font-size:18px;font-weight:bold;text-align:center'>Message</div>"
			//       var content_msg="<div style='text-align:left;margin-top:10px;padding:5px;'> Please enter text for marketing material.</div>";
			//       var footer_msg="<div><hr><input type='button'  value='Ok' id='popupcancel' name='popupcancel' style='cursor:pointer;background-color:#E6E6E6 !important;background-image:none !important;color:#000;border:1px solid #D3D3D3;border-radius:4px 4px 4px 4px;'></div>";
			//       jQuery( "#dialog-message" ).html(head_msg + content_msg +footer_msg);
			//       jQuery.fancybox({
			//		    content:jQuery('#dialog-message').html(),
			...
			else
			{
				...
				//alert( "getmarketingmaterial=yes&locid="+lid+"&campid=<?php if(isset($_REQUEST['id'])) echo $_REQUEST['id'] ?>&image="+ jQuery("#hdn_image_path").val()+"&text="+jQuery("#txt_material").val()+"&size_id="+jQuery("#document_size").val());
			
				jQuery.ajax({
					type: "POST",
					url: "<?=WEB_PATH?>/merchant/process.php",
					//data: "getmarketingmaterial=yes&locid="+lid+"&campid=<?php if(isset($_REQUEST['id'])) echo $_REQUEST['id'] ?>&image="+ jQuery("#hdn_image_path").val()+"&text="+jQuery("#txt_material").val()+"&size_id="+jQuery("#document_size").val()+"&merchant_id=<?php echo $_SESSION['merchant_id']; ?>",
					data: "getmarketingmaterial=yes&locid="+lid+"&campid=<?php if(isset($_REQUEST['activationcode'])){echo $_REQUEST['id'];}else{echo '0';} ?>&image="+ jQuery("#hdn_image_path").val()+"&text="+jQuery("#txt_material").val()+"&size_id="+jQuery("#document_size").val()+"&merchant_id=<?php echo $_SESSION['merchant_id']; ?>",
					async : false,
					success: function(msg) {
						var obj = jQuery.parseJSON(msg);
						...
						jQuery.fancybox({
							...
							afterShow:function(){
								...
								//                       jQuery(".fancybox-inner #description_content").css("height","161px");
								//                        jQuery(".fancybox-inner #description_content").css("overflow","hidden");
								//                        jQuery(".div_preview #description_content").css("width","416px");
						...
						//    
						//    jQuery(".fancybox-wrap").css({ top: 0, left: left});
						//// jQuery(".fancybox-inner").css("width","800px !important");
					}
			});
		}
        return false;
	...
	jQuery.ajax({
		...
		success:function(msg)
		{
			...
			else
			{
				...
				//          //alert("Enter Text To Print On Your Marketing Material");
				//	  var head_msg="<div style='line-height:2;height:40px;width:100%;border-radius:4px 4px 4px 4px;background:#D5D5D5;font-size:18px;font-weight:bold;text-align:center'>Message</div>"
				//       var content_msg="<div style='text-align:left;margin-top:10px;padding:5px;'>Please enter text To Print On Your Marketing Material</div>";
				//       var footer_msg="<div><hr><input type='button'  value='Ok' id='popupcancel' name='popupcancel' style='cursor:pointer;background-color:#E6E6E6 !important;background-image:none !important;color:#000;border:1px solid #D3D3D3;border-radius:4px 4px 4px 4px;'></div>";
				//       jQuery( "#dialog-message" ).html(head_msg + content_msg +footer_msg);
				//       jQuery.fancybox({
				//		    content:jQuery('#dialog-message').html(),
				...
				//          return false;
				//      }
				//  alert(jQuery("#document_size").val());
				if(jQuery("#hdn_image_path").val()=="" )
				{
					//alert("Select Image For Your Marketing Material");
					var head_msg="<div class='head_msg'>Message</div>"
				...
				else
				{
					...
					jQuery.ajax({
						type: "POST",
						url: "<?=WEB_PATH?>/merchant/process.php",
						data: "getmarketingmaterial=yes&locid="+lid+"&campid=<?php if(isset($_REQUEST['id']))echo $_REQUEST['id'] ?>&image="+ jQuery("#hdn_image_path").val()+"&text="+jQuery("#txt_material").val()+"&size_id="+jQuery("#document_size").val(),
						async : false,
						success: function(msg) {
							//  alert(msg);
							//    jQuery(".QRcode_detail_div").html(msg);
							...
							// alert(obj.location_image);
							jQuery(".div_preview").html(obj.html_content);
							///  alert(obj.html_content);
							// alert( jQuery(".div_preview img[id='img_campaign']").length);
							//   jQuery(".div_preview img[id='img_campaign']").attr("src","<?php echo ASSETS_IMG ?>/m/logo/"; ?>"+jQuery("#hdn_image_path").val());
							jQuery("#img_qrcode").attr("src","<?php echo WEB_PATH.'/QR-Generator-PHP-master/php/qr.php?d=';?>"+obj.qrcode+"&size=180");
							jQuery("#img_location").attr("src",obj.location_image);
							jQuery("#description_content").html(tinyMCE.activeEditor.getContent());
							jQuery("#location_address").html(obj.location_address);
							//  alert( jQuery("#appstore-image").length);
							jQuery("#appstore-image").attr("src","<?php echo ASSETS_IMG ?>/m/app-store.png");
							jQuery("#anroid-image").attr("src","<?php echo ASSETS_IMG ?>/m/anroid.png");
							jQuery("#scanflip-logo").attr("src","<?php echo ASSETS_IMG ?>/m/scanplip-logo.png");

							///   jQuery(".div_preview .main-outer-table").css("width","720px");
							/// jQuery(".div_preview .main-outer-table").css("height","900px");
							jQuery(".div_preview #location_address").parent().css("text-align","left");
							//    jQuery(".div_preview #description_content").css("height","161px");
							jQuery(".div_preview #description_content").css("overflow","hidden");

							// jQuery(".div_preview #description_content").css("width","416px");
							//alert("<?php echo WEB_PATH ?>/merchant/demopdf/demo_template.php?html="+encodeURIComponent(jQuery(".div_preview").html()));
							// setCookie('material_html',jQuery(".div_preview").html(),365);
							//alert(getCookie('material_html'));
