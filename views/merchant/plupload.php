	...
	{		
		...
		if($width>=400 && $height>=310)
		{
			...
			if($width>=$height)
			{
				...
				$upload_filename = $image_folder . $name;					
				passthru("/usr/bin/convert -strip -interlace Plane -quality 95% -define jpeg:size=800x600 ".UPLOAD_IMG."/m/".$target."/".$filename." \
		  -thumbnail ".$max_width."x".$height."  -unsharp 0x.5 ".$upload_filename);
				//passthru("/usr/bin/mogrify -strip ".$upload_filename);
			}
			else
			{
				...
				$upload_filename = $image_folder . $name;					
				passthru("/usr/bin/convert -strip -interlace Plane -quality 95% -define jpeg:size=800x600 ".UPLOAD_IMG."/m/".$target."/".$filename." \
		  -thumbnail ".$width."x".$max_height."  -unsharp 0x.5 ".$upload_filename);	
				//passthru("/usr/bin/mogrify -strip ".$upload_filename);
		...
		{	   
			if($height<$max_height) 
			{
				...
				$upload_filename = $image_folder . $name;					
				passthru("/usr/bin/convert -strip -interlace Plane -quality 95% -define jpeg:size=800x600 ".UPLOAD_IMG."/m/".$target."/".$filename." \
		  -thumbnail 400x".$height."  -unsharp 0x.5 ".$upload_filename);	
				//passthru("/usr/bin/mogrify -strip ".$upload_filename);
		...
		{		
			if($width < $max_width)
			{
				...
				$upload_filename = $image_folder . $name;					
				passthru("/usr/bin/convert -strip -interlace Plane -quality 95% -define jpeg:size=800x600 ".UPLOAD_IMG."/m/".$target."/".$filename." \
		  -thumbnail ".$width."x310  -unsharp 0x.5 ".$upload_filename);
				//passthru("/usr/bin/mogrify -strip ".$upload_filename);
		...
		{
			unlink(UPLOAD_IMG.'/m/'.$target.'/'.$name);
			die('{"jsonrpc" : "2.0", "error" : {"code": 502, "message": "image size must be at-least ( W X H) 400px x 200px."}, "id" : "id"}');
		} 	
	}
	else if($_REQUEST['image_type']=="3")
	{
		if($width<1600 && $height<1200)
		{
			unlink(UPLOAD_IMG.'/m/'.$target.'/'.$name);
			die('{"jsonrpc" : "2.0", "error" : {"code": 503, "message": "Failed to load image. Giftcard listing looks best with photo atleast 1600px wide and 1200px high."}, "id" : "id"}');
		}
		else
		{
			$aspect_ratio=$width/$height;
...
{
	unlink(UPLOAD_IMG.'/m/'.$target.'/'.$name);
	die('{"jsonrpc" : "2.0", "error" : {"code": 500, "message": "Sorry, you can\'t upload more than '. $get_billing_pack_data_mu_old1->fields['image_upload_limit'].' images."}, "id" : "id"}');
}
