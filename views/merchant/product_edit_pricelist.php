...
if(isset($_POST['id'])){
  ...
  if ( $SelectFile) {
	...
	echo '<div class="hidden">
		<input type="hidden" value="'.$Row[0].'" id="pricelistid">
		<input type="hidden" value="'.$Row[1].'" id="pricelistname">
	  </div>';
  }	
}
else
...
// start memcache
$json = $memcached->get("pricelistjson_".$_POST['id']);
if(empty($json))
{
	//echo "not in memcache";
	//$json = file_get_contents(ASSETS.'/pricelist/json/'.$filename.'_'.$pricelist_id.'.json');
	$json = file_get_contents(ASSETS.'/pricelist/json/'.$pricelist_id.'.json');
...
<html lang="en">
	<head>
		...
		<script src="js/product_edit_pricelist.js"></script>
...
<body>
    ...
    <div class="container">
      <div class="clearfix">
        ...
        <h3 class="pricelist_name fleft"></h3>
      ...
      <button type="button" class="btn btn-primary" id="close_pricelist" onclick="close_confirm('update')">Close</button>
    ...
    <?php 
      echo '<form action="updatedata.php" method="post" id="post_json">
              <input id="json_input" type="hidden" name="json">
              <input type="hidden" name="pricelist_name_val" id="pricelist_name_val">
              <input type="hidden" name="pricelist_id" value="'.$Row[0].'" id="pricelist_id">
...
// product_edit_pricelist.js	
$(document).ready(function(){
	var filename = $('#pricelistname').attr('value');
	var pricelist_id = $('#pricelistid').attr('value');
	var select_image_text = 'Select image';
	/*
	//alert("<?php echo ASSETS.'/pricelist/json/' ?>"+filename+"_"+pricelist_id+".json");
	$.ajax({
	  //url: "<?php echo ASSETS.'/pricelist/json/' ?>"+filename+"_"+pricelist_id+".json",
	   url:'<?php echo WEB_PATH; ?>/merchant/process.php',
	  data :'getpricelistjsonfile=yes&filename='+filename+'&pricelist_id='+pricelist_id,
	...
	*/
		...
		/*	
		 ...
		 //newdata = newdata.replace("\r","\\r");
		*/ 
		
		 var data = '<?php echo $json; ?>';
		  console.log(data);
			
		//data = data.replace(/dq/g, '"');
		 ...
		 console.log(data);
		 //data = data.replace(/&amp;/g, '\&');
		 //data='{"pricelist_id":"80","pricelist_name":"np1","category": [{"category_name": "a&amp;","hide_category": "false"},{"category_name": "Abc bsdd #$%&amp;*/\","hide_category": "false"}]}';
		 //data='{"pricelist_id":"80","pricelist_name":"np1","category": [{"category_name": "a&amp;","hide_category": "false"},{"category_name": "Abc bsdd #$%&amp;*/\","hide_category": "false"}]}';

		 //console.log('from static');
		 //console.log(data);
		 
		 //data = '{"pricelist_id":"62","pricelist_name":"asd","category": [{"category_name": "mea't","hide_category": "false"}]}';
		 //data = '{"pricelist_id":"62","pricelist_name":"asd","category": [{"category_name": "mea\'t","hide_category": "false"}]}';
		 var newdata = data;
		 newdata = newdata.replace(/\s+/g," "); 
		 //newdata = newdata.replace("\n","\\n");
		...
		//console.log('hello');
		var pricelist_name= data.pricelist_name;
		pricelist_name=pricelist_name.replace(/dq/g, '"');
		//console.log(pricelist_name);
		$('.pricelist_name').html(pricelist_name);
		$('#pricelist_name').val(he.unescape(pricelist_name));
		//console.log('pricelist_name');


		//var choicecategory_count = data.choice_category.length;
		if(typeof(data.choice_category) != "undefined"){
			var choicecategory_count= data.choice_category.length;
			var choicecategory_id;
			var choicecategory_name;
			var choicecategory_choices;
			for (i=0;i<choicecategory_count;i++){
			choicecategory_id = data.choice_category[i].choice_category_id;
			choicecategory_name = data.choice_category[i].choice_category_name;
			choicecategory_choices = data.choice_category[i].choice_category_choices;			
			console.log(choicecategory_choices);

			$('.choicecategories_group').append('<div class="choicecategory_box clearfix">\
		        <div class="clearfix">\
			...
			$('.choicecategories_group').find('.choicecategory_box').first().find('.choice_container').val(choicecategory_choices)
			$('.choice_container').tagsinput();
		}			
		}
		
		if(typeof(data.category) != "undefined"){
			var category_count = data.category.length;
			
			for(j=0;j<category_count;j++){
				//console.log(data.category[j].category_name)
				var category_name = data.category[j].category_name;
				category_name=category_name.replace(/dq/g, '"');
				var hide_category= data.category[j].hide_category;
				$('.pricelist_container').find('.categories_group').append('<div class="category_container">\
				...
				if(typeof(data.category[j].note) != "undefined"){
					var notes_description = data.category[j].note;
					notes_description=notes_description.replace(/dq/g, '"');
					$('.categories_group').find('.category_container').eq(j).find('.category_notes_container').prepend('<li class="note clearfix">\
			    ...
			    if(typeof(data.category[j].items) != "undefined"){
		        	var item_count=data.category[j].items.length;
		        	console.log(item_count);
		        	for(m=0;m<item_count;m++){
		        		var item_name = data.category[j].items[m].item_name;
		        		item_name=item_name.replace(/dq/g, '"');
		        		var item_description = data.category[j].items[m].item_description;
		        		item_description=item_description.replace(/dq/g, '"');
		        		var item_price = data.category[j].items[m].item_price;
		        		var display_item_price = data.category[j].items[m].hide_item_price;
		        		var selected_choicecategory_id = data.category[j].items[m].choicecategory_id;
		        		var image1_url = data.category[j].items[m].image1;
		        		var image2_url = data.category[j].items[m].image2;
		        		var image3_url = data.category[j].items[m].image3;
	        			var image1_tag;
		        		if (image1_url.length!=0 && image1_url!= 'undefined'){
		        			image1_tag='<img src="'+image1_url+'"/>';
		        		}
		        		else{
		        			image1_tag = '';
		        		}
		        		if (image2_url.length!=0 && image2_url!= 'undefined'){
		        			image2_tag='<img src="'+image2_url+'"/>';
		        		}
		        		else{
		        			image2_tag = '';
		        		}
		        		if (image3_url.length!=0 && image3_url!= 'undefined'){
		        			image3_tag='<img src="'+image3_url+'"/>';
		        		}
		        		else{
		        			image3_tag = '';
		        		}

			        	$('.categories_group').find('.category_container').eq(j).find('.category_item_container').append('<li class="item clearfix">\
				          <div class="item_nav clearfix">\
				            <div class="item_details fleft">\
				              ...
				              <div class="images_container clearfix">\
				                <div class="image1">'+image1_tag+'</div>\
				                <div class="image2">'+image2_tag+'</div>\
				                <div class="image3">'+image3_tag+'</div>\
				              </div>\
				            </div>\
				          ...
				          <div class="master_container">\
				            ...
				            <div class="option_form">\
				              ...
				              <input class="option_price form-control" placeholder="0.00"  type="text" maxlength="7">\
				              <p class="hint">'+price_hint+'</p>\
				              <div class="checkbox" >\
				                <label>\
						...
						if(typeof(data.category[j].items[m].options) != "undefined"){
							var option_count = data.category[j].items[m].options.length;
							for(o=0; o<option_count;o++){
								var option_name=data.category[j].items[m].options[o].option_name;
								option_name=option_name.replace(/dq/g, '"');
								var option_price = data.category[j].items[m].options[o].option_price;
								var hide_option_price = data.category[j].items[m].options[o].hide_option_price;
								$('.categories_group').find('.category_container').eq(j).find('.item').eq(m).find('.option_container').append('\
				...
				if(typeof(data.category[j].subcategories) != "undefined"){
					var subcategory_count = data.category[j].subcategories.length;
					for(k=0;k<subcategory_count;k++){

						var subcategory_name = data.category[j].subcategories[k].subcategory_name;
						subcategory_name=subcategory_name.replace(/dq/g, '"');
						$('.categories_group').find('.category_container').eq(j).find('.subcategory_container').append('\
				        ...
				        if(typeof(data.category[j].subcategories[k].items) != "undefined"){
				        	var item_count=data.category[j].subcategories[k].items.length;
				        	console.log(item_count);
				        	for(l=0;l<item_count;l++){
				        		var item_name = data.category[j].subcategories[k].items[l].item_name;
				        		item_name=item_name.replace(/dq/g, '"');
				        		var item_description = data.category[j].subcategories[k].items[l].item_description;
				        		item_description=item_description.replace(/dq/g, '"');
				        		var item_price = data.category[j].subcategories[k].items[l].item_price;
				        		var display_item_price = data.category[j].subcategories[k].items[l].hide_item_price;
				        		var selected_choicecategory_id = data.category[j].subcategories[k].items[l].choicecategory_id;
				        		var image1_url = data.category[j].subcategories[k].items[l].image1;
				        		var image2_url = data.category[j].subcategories[k].items[l].image2;
				        		var image3_url = data.category[j].subcategories[k].items[l].image3;

					        	$('.categories_group').find('.category_container').eq(j).find('.subcategory_container').eq(k).find('.subcategory_item_container').append('<li class="item clearfix">\
						          <div class="item_nav clearfix">\
						            <div class="item_details fleft">\
						              <span class="glyphicon glyphicon-plus fleft item_expand"></span>\
						              <span class="glyphicon glyphicon-minus fleft item_collapse"></span>\
								...
								if(typeof(data.category[j].subcategories[k].items[l].options) != "undefined"){
									var option_count = data.category[j].subcategories[k].items[l].options.length;
									for(n=0; n<option_count;n++){
										var option_name=data.category[j].subcategories[k].items[l].options[n].option_name;
										option_name=option_name.replace(/dq/g, '"');
										var option_price = data.category[j].subcategories[k].items[l].options[n].option_price;
										var hide_option_price = data.category[j].subcategories[k].items[l].options[n].hide_option_price;
										$('.categories_group').find('.category_container').eq(j).find('.subcategory_container').eq(k).find('.item').eq(l).find('.option_container').append('\
								        <div class="option clearfix">\
								          <div class="option_details clearfix">\
								            <div class=" fleft clearfix option_titlebar"> \
								              <h4 class="option_name_val fleft ">'+option_name+'</h4>\
...
function edit_item(){
  ...
  $('body').on('click','.edit_item',function(){
    ...
    $(this).closest('.item').find('.item_name').val(he.unescape($(this).closest('.item').find('.item_name_val').html()));
    ...
    $(this).closest('.item').find('.item_description').val(he.unescape($(this).closest('.item').find('.item_description_val').html()));
...
function edit_category(){
  ...
  $('body').on('click','.edit_category',function(){
	  var cn= he.unescape($(this).closest('.category_container').find('.category_name_val').html());
	  console.log("old "+cn);
	  cn=cn.replace(/dq/g, '"');
	  console.log("new "+cn);
  $(this).parent().parent().find('.categoryname').val(cn);
