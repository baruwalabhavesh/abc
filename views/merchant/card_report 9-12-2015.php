	...
	if(isset($_REQUEST['opt_filter_days'])) // after filter
	{
		...
		if($RS->RecordCount()>0)
		{
			if($days!=0)
			{
				...
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=1 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.")) total_male_activated_cards,
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=1 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.") and uc.device_activation_mode=1) total_male_activated_desktop_cards, 
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=1 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.") and uc.device_activation_mode=2) total_male_activated_mobile_cards, 
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=1 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.") and uc.device_activation_mode=3) total_male_activated_qr_cards, 
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=2 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.")) total_female_activated_cards,
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=2 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.") and uc.device_activation_mode=1) total_female_activated_desktop_cards,
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=2 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.") and uc.device_activation_mode=2) total_female_activated_mobile_cards,
				(select count(*) from customer_loyalty_reward_card uc,customer_user cu where uc.user_id=cu.id and cu.gender=2 and uc.card_status=1 and uc.card_id=".$card_id." and (DATEDIFF('".date("Y-m-d H:i:s")."',uc.date_activated ) <= ".$days.") and uc.device_activation_mode=3) total_female_activated_qr_cards,
