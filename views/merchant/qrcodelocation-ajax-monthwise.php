...
/**
 * @uses qrcode location month wise
...
foreach ($months as $month) {
        ...
        $month_expr = " and a.scaned_date between '" . $from_date . "' and '" . $to_date . "' ";
        /* $sql = " select distinct a.* ,b.location_name ,b.address ,b.state,b.city,b.zip,b.created_by  ,b.id location_id, (select gender c from customer_user c where  c.id=a.user_id) gender
          from scan_qrcode a inner join locations b on a.location_id = b.id
          where b.id=".$_REQUEST['selected_locations']."   ".$month_expr . "  ".$merchant_sql;
          $RS_location_tot = $objDB->execute_query($sql); */
        
        $RS_location_tot = $objDB->Conn->Execute(" select distinct a.* ,b.location_name ,b.address ,b.state,b.city,b.zip,b.created_by  ,b.id location_id, (select gender c from customer_user c where  c.id=a.user_id) gender
               from scan_qrcode a inner join locations b on a.location_id = b.id 
			   where b.id=?   " . $month_expr . "  " . $merchant_sql, array($_REQUEST['selected_locations']));
        $mnt = $month - 1;
        
        if ($RS_location_tot->RecordCount() == 0) {
                        ...
                        if (array_key_exists($Row_qrcode1['location_id'], $location_arr)) {
                                ...
                                if (in_array($Row_qrcode1['location_id'], $selected_loations)) {
                                        ...
                                        if ($Row_qrcode1['is_unique'] == 1) {
                                                ...
                                                $unique_qrcodelocation_arr[$mnt] = $unique_qrcodelocation_arr[$mnt] + 1;
...
$from_date = $yr . "-01-01 00:00:00";
$to_date = $yr . "-12-31 23:59:59";
...
$unique_qrcodetotal_arr = $unique_qrcodetotal_arr;
$unique_qrcodelocation_arr = $unique_qrcodelocation_arr;
$json_array['locatino_json'] = $locatino_json;
