...
<html lang="en">
	<head>
		...
		<script src="js/service_edit_pricelist.js"></script>
...
<body>
	...
	<div id="content">
  	<!--<div class="edit_service_pricelist"> -->
...
// service_edit_pricelist.js
//repopulating form into edit page
$(document).ready(function(){
  ...
  */
    ...
    if(typeof(data.category) != "undefined"){
      ...
      //looping through categories in json and repopulating
      for(j=0;j<category_count;j++){
        ...
        $('.pricelist_container').find('.categories_group').append('<div class="category_container">\
            ...
            <div class="error_messages">\
              ...
              <div class="alert alert-warning alert_item_name">Please enter Service name</div>\
            ...
            <div class="master_container">\
              <div class="category_item_form" >\
               ...
               <input type="text" class="duration form-control" />\
        ...
        if(typeof(data.category[j].note) != "undefined"){
          ...
          if(typeof(data.category[j].services) != "undefined"){
              var item_count=data.category[j].services.length;
              console.log(item_count);
              //looping json and adding services
              for(m=0;m<item_count;m++){
                var item_name = data.category[j].services[m].service_name;
                item_name=item_name.replace(/dq/g, '"');
                var item_description = data.category[j].services[m].service_description;
                item_description=item_description.replace(/dq/g, '"');
                var item_price = data.category[j].services[m].service_price;
                var display_item_price = data.category[j].services[m].hide_service_price;
                var display_item_duration = data.category[j].services[m].hide_service_duration;
                var service_duration = data.category[j].services[m].service_duration;
                $('.categories_group').find('.category_container').eq(j).find('.category_item_container').append('<li class="item clearfix">\
                  <div class="item_nav clearfix">\
                    <div class="item_details fleft">\
                      ...
                      <p class="item_duration hidden_div">'+service_duration+'</p>\
                    </div>\
          ...
          if(category_item_count>0){
            ...
            for(k=0;k<category_item_count;k++){
              ...
              console.log("hide_item_price : "+hide_item_price);
               ...
               {
				    console.log("if hide_item_price");
					item_node ="";
               }
               else
               {
					...
					console.log("else hide_item_price");
					item_node='<p class="item_price fright">'+item_price_val+'</p>';
               }
               console.log("item_node : "+item_node);
              ...
              preview_dom_elements += '<li class="clearfix">\
                  <div class="iteminfo clearfix fleft">\
                    ...
                    //hide_item_duration = hide_item_duration.trim();
			...
			for(i=0; i<category_count;i++){
				...
				if(category_item_count>0){
					...
					for(k=0;k<category_item_count;k++){
						...
						var hide_item_duration =  $('.category_container').eq(i).find('.category_item_container').find('.item').eq(k).find('.hide_item_duration').attr('value');
						var item_duration =  $('.category_container').eq(i).find('.category_item_container').find('.item').eq(k).find('.item_duration').html();
						//alert(hide_item_price);
						//alert(hide_item_duration);
...
function edit_item(){
  ...
  $('body').on('click','.edit_item',function(){
    ...
    //alert($(this).closest('.item').find(' p.hide_item_duration').attr('value'));
...
function edit_category(){
  ...
  $(this).parent().parent().find('.categoryname').val(he.unescape( $(this).closest('.category_container').find('.category_name_val').html()));
