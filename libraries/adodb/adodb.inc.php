...
class ADOConnection
{
	...
	function AutoExecute($table,$data,$type,$criteria=''){
		$result = $this->Execute("desc " . $table) or $return=0;
		if($type == "INSERT")
		$start = "insert into " . $table . " set ";
		elseif($type == "UPDATE")
		$start = "update " . $table . " set ";
		$sql = $start;
		while($rst = $result->FetchRow())
		{
			foreach($data as $key=>$value)
			{
				if($key==$rst['Field'] and $key!==0)
				{
					if((!ereg('int',$rst['Type'])))
						$sql = $sql . $key . "='" . $value . "', ";
					else{
						if (trim($value)!=""){
							$sql = $sql . $key . "=" . $value . ", ";
		...
		if($this->Execute($sql) or $return = 0)
		$return = 1;
		return $return;
	}
